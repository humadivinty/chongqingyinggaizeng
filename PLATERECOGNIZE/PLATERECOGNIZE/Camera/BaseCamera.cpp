    ...
    LPCSTR szVideoExtInfo)
{
    ...
    m_iVideoIndex++;
    if (m_iVideoIndex > 400)
        m_iVideoIndex = 0;

    m_curH264Ms = dw64TimeMS;

    CustH264Struct* pH264Data = new CustH264Struct(pbVideoData, dwVideoDataLen, dwWidth, dwHeight, isIFrame, isHistory, dw64TimeMS/* GetTickCount64()*/, m_iVideoIndex);
...
bool BaseCamera::WriteLog(const char* chlog)
{
    ...
    if (strlen(chLogRoot) <= 0 || 0 == strcmp(chLogRoot, "0"))
    {
...
void BaseCamera::SendMessageToPlateServer(int iMessageType /*= 1*/)
{
    ...
    addrSrv.sin_port = htons((USHORT)VEHICLE_LISTEN_PORT);
...
bool BaseCamera::StopSaveAviFile(int iStreamID, INT64 TimeFlag /*= 0*/)
{
    if (m_hHvHandle == NULL)
    {
