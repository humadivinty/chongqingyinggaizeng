...
#include "HvCamera.h"
//#include "HvDevice/Camera6467_plate.h"
...
void Camera6467_plate::AnalysisAppendInfo(CameraResult* record)
{
    ...
    else
    {
        ...
        char tmpPlate[32] = { 0 };
        strncpy_s(tmpPlate, record->chPlateNO, strlen(record->chPlateNO));
        ...
        else if (strstr(tmpPlateColor, "绿"))
        {
            ...
            {
                record->iPlateColor = COLOR_YELLOW_GREEN;
        ...
        //获取车牌号
        char sztempPlate[32] = { 0 };
        sprintf_s(sztempPlate, "%s", record->chPlateNO + 2);
        //sprintf_s(sztempPlate, "%s", record->chPlateNO);
        if (NULL != sztempPlate)
        {            
            //if (strstr(tmpPlateColor, "绿"))
            //{
            //    memset(record->chPlateNO, 0, strlen(record->chPlateNO));
            //    if (record->iPlateColor = COLOR_YELLOW_GREEN)
            //    {
            //        sprintf_s(record->chPlateNO, "黄绿双拼%s", sztempPlate);
            //    }
            //    else
            //    {
            //        sprintf_s(record->chPlateNO, "渐变绿%s", sztempPlate);
            //    }
            //}
            memset(record->chPlateNO, 0, strlen(record->chPlateNO));
            sprintf_s(record->chPlateNO, 32, "%s", sztempPlate);
        }
    }
    char * pchObj = NULL;
    ...
    if (m_pVideoImgBuffer)
    {
        ...
        if (H264_GetOneBmpImg((HANDLE)m_hVideo, m_pVideoImgBuffer, iImgBufferSize, iImgWidth, iImgHeight))
        {
            ...
            else
            {
                ...
                if (camImg.dwImgSize > (DWORD)iOrigeLength)
                {
